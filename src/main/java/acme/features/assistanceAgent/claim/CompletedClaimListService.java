...
public class CompletedClaimListService extends AbstractGuiService<AssistanceAgent, Claim> {
	...
	public void load() {
		...
		claims = this.repository.findClaimsByAssistanceAgent(assistanceAgentId).stream().filter(x -> x.getIndicator() != ClaimStatus.PENDING).toList();
	...
	public void unbind(final Claim claim) {
		...
		Dataset dataset;
		ClaimStatus indicator = claim.getIndicator();

		dataset = super.unbindObject(claim, "passengerEmail", "type");
		dataset.put("indicator", indicator);
		super.addPayload(dataset, claim, "registrationMoment", "description", "leg.flightNumberDigits");
